...
sql_business="""select case when business_name is not null then business_name else '未知' end business_name,
count(case when is_activate=1 then 1 else null end) business_activate,
count(case when is_edu=1 and is_activate=1 then 1 else null END) business_edu,
count(case when is_ec=1 and is_activate=1 then 1 else null end)business_ec,
count(*) business_all 
from members group by business_name order by business_activate  desc"""