...
sql_edu_xiaoe="""select count(distinct order_id) sales_count,sum(price) sales_amount,count(distinct user_id)sales_buyers ,
CAST(sum(price)*0.3 as UNSIGNED) 
from edu_orders_xiaoe where order_state=1 and date(created_at) between '2018-07-01' and '%s'"""

sql_edu_800vip="""select class_title,count(distinct order_id )'日销量',ifnull(sum( orderamount + wallet_pay ),0)'日流水' 
from  edu_orders 
where  order_status='success' and pay_time between '{0}'and '{1}'and product_id='4391'"""
...
cast(sum((orderamount + wallet_pay)*ifnull(b.proportion,0)) as unsigned)  '收入分成'
 from edu_orders a left join edu_income_proportion b on a.product_id=b.product_id   
 where  order_status='success' and pay_time between '{0}' and '{1}';"""
sql_edu_1day="""select 
count(distinct  order_id )'日销量',
ifnull(sum( orderamount + wallet_pay ),0)'日流水'
 from edu_orders a  where order_status='success' and pay_time between '{0}' and '{1}';"""
...
from edu_orders 
where order_status='success' and pay_time between '{0}' and '{1}'
 group by date(pay_time) order by date(pay_time) desc """
...
from edu_orders a left join edu_income_proportion b on a.product_id=b.product_id 
where order_status='success'and pay_time between '{0}' and '{1}'
group by class_title order by class_sale_count desc,teacher_name desc"""
...
,count(distinct case when year(pay_time)=2019 then order_id else null end) '2019月销量' ,
cast(sum( case when year(pay_time)=2019 then orderamount+wallet_pay else null end) as UNSIGNED)  '2019月流水'
from edu_orders where  order_status='success'  group by month(pay_time) order by month(pay_time)  """

sql_edu_class_all="""select a.class_title,teacher_name,count(distinct order_id) class_sale_count,
round((sum(orderamount)+sum(wallet_pay))/count(distinct order_id),0),
...
sql_littleclass_total="""select sum(login_uv) login_uv,sum(deblock_count) deblock_count,sum(b.homeworkers),
sum(deblock_count)/sum(login_uv) cvr_deblock,
sum(b.homeworkers)/sum(login_uv) cvr_homework  from ( 
 select a.post_id,a.post_title,total_login_uv login_uv,
 count(distinct b.payment_id) deblock_count ,
 count(distinct c.member_id) homeworkers,
 count(distinct b.payment_id)/total_login_uv cvr_deblock,
 count(distinct c.member_id)/total_login_uv cvr_homework
 ...
 where b.post_id is not null or c.post_id is not null 
 group by a.post_id)b order by login_uv desc;"""
...
count(*)'city_members'
from members 
where edu_time between '{0}' and '{1}' and is_edu=1  group by city_edu order by count(*) desc limit 10;"""