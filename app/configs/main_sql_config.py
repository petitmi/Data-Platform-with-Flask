...
left join finances_sum b on a.department_id=b.department_id  where  year(b.received_time) =2018 or b.received_time is null 
group by a.department_name order by a.order_id;"""
...
where sales_amount>0  and year(b.received_time) =2018
...
where brand_name='拍片学院'  and year(pay_time)=2018 group by month)b on a.month=b.month """
sql_ppxy_month="""select a.month,ifnull(sales_amount_edu,0) sales_amount_edu
from month_list a left join
(select month(pay_time) month ,sum( orderamount + wallet_pay ) sales_amount_edu from  edu_orders 
where brand_name='CSC电影学院' and year(pay_time)=2018 group by month)b on a.month=b.month """
sql_ec_month_chanjet="""select a.month,ifnull(sales_amount_consult,0) sales_amount_consult
from month_list a left join
(select month(order_date) month,sum(price_tax_sell*goods_num) sales_amount_consult 
from ec_orders_chanjet group by month(order_date))b on a.month=b.month"""
sql_ec_month_offline="""select a.month,ifnull(sales_amount_consult,0) sales_amount_consult
from month_list a left join
(select month,sales_amount sales_amount_consult from ec_month_offline where year=2018 and month<7 order by month)b on a.month=b.month"""
sql_ec_month_online="""select a.month,ifnull(sales_amount_online,0) sales_amount_online
from month_list a left join
(select month(pay_time) month,cast(sum(pay_amount) as UNSIGNED) sales_amount_online
from ec_orders where order_state=1 and  year(pay_time)=2018  group by month(pay_time) order by month(pay_time))b on a.month=b.month"""
sql_littleclass_month="""select a.month,ifnull(sales_amount_edu,0) sales_amount_edu
from month_list a left join
(select month(pay_time) month,sum(sales_amount) sales_amount_edu from littleclass_orders where year(pay_time)=2018  
group by month(pay_time) order by month(pay_time))b on a.month=b.month"""


sql_ec_chanjet_thismonth="""select sum(price_tax_sell*goods_num) sales_amount  from ec_orders_chanjet where date(order_date) between '%s' and '%s'"""
sql_ec_online_thismonth="""select cast(sum(pay_amount) as UNSIGNED) sales_amount from ec_orders where order_state=1 and  date(pay_time) between '%s' and '%s' """
sql_csc_thismonth="""select sum( orderamount + wallet_pay ) sales_amount from edu_orders where brand_name='拍片学院' and order_status='success'  and date(pay_time) between '%s' and '%s'"""
sql_ppxy_thismonth="""select sum( orderamount + wallet_pay ) sales_amount from edu_orders where brand_name='CSC电影学院' and order_status='success'   and date(pay_time) between '%s' and '%s'"""
sql_littleclass_thismonth="""select sum(sales_amount) sales_amount from littleclass_orders where date(pay_time) between '%s' and '%s'"""
sql_project_thismonth="""select a.department_name,a.project_name,ifnull(sum(case when date(b.received_time) between '%s' and '%s' then sales_amount else null end),0) sales_amount
...
sql_ec_chanjet="""select sum(price_tax_sell*goods_num) sales_amount  from ec_orders_chanjet"""
sql_ec_offline="""select sum(sales_amount) sales_amount from ec_month_offline where year=2018 and month<7 """
sql_ec_online="""select cast(sum(pay_amount) as UNSIGNED) sales_amount from ec_orders where order_state=1 and  year(pay_time)=2018 """
sql_csc="""select sum( orderamount + wallet_pay ) sales_amount from edu_orders where brand_name='拍片学院' and order_status='success'  and year(pay_time)=2018"""
sql_ppxy="""select sum( orderamount + wallet_pay ) sales_amount from edu_orders 
where brand_name='CSC电影学院' and order_status='success' and year(pay_time)=2018"""
sql_littleclass="""select sum(sales_amount) sales_amount  from littleclass_orders where year(pay_time) =2018"""
sql_project="""select a.department_name,a.project_name,ifnull(sum(sales_amount),0) sales_amount
from finances_type a left join finances_sum b on a.project_id=b.project_id where  year(b.received_time) =2018 or b.received_time is null 
group by a.project_name 
order by a.order_id ,a.project_id desc;"""
...
where year(order_date)=2018 group by month(order_date) )b on a.month=b.month;"""
sql_ec_offline_month_compared="""select a.month,ifnull(b.sales_amount,0) sales_amount_ec_offline from month_list a left join 
(select month,sales_amount from ec_month_offline where year=2018 and month<7)b on a.month=b.month;"""
...
where order_state=1 and  year(pay_time)=2018 
...
where brand_name='拍片学院' and order_status='success'  and year(pay_time) =2018 group by month(pay_time))b on a.month=b.month;"""
sql_ppxy_month_compared="""select a.month,ifnull(b.sales_amount,0) sales_amount_ppxy from month_list a left join 
(select month(pay_time) month,sum( orderamount + wallet_pay ) sales_amount from edu_orders 
where brand_name='CSC电影学院' and order_status='success'   and year(pay_time)=2018 group by month(pay_time))b on a.month=b.month;"""
...
where year(pay_time) =2018 group by month(pay_time))b on a.month=b.month;"""
...
from finances_sum where year(received_time)=2018
